...
* カタログのスレにマウスオーバーすると\[NG\]ボタンが表示されます。このボタンをクリックするとメニューが表示されます。  
  ...
  - 「スレNG」をクリックするとそのスレが一時的に非表示となります。  
    「スレNG」にするとスレNG\[クリア\]ボタンでクリアするまではそのスレが非表示になります。（リロードでは解除されません）  
...
* NG機能\[一時無効\]ボタンをクリックするとNGスレが一時的に表示されます。  
    ![スクリーンショット](images/screenshot06.png)  
  - もう一度\[一時無効\]ボタンをクリックするかリロードするとNGスレが消えます。  
  - NG機能一時無効中は\[NG\]ボタンからのNG登録ができません。  
...
* MAX\_NG\_THREADS : NGスレの最大保持数（デフォルト：512）  
  - 記憶可能なNGスレの最大数です。NGスレの記憶数が設定値を超えると古い順に破棄されます。  
* MAX\_REGISTERED\_NG\_IMAGES : NG画像の最大登録数（デフォルト：2048）  
  - 登録可能なNG画像の最大数です。NG画像の登録数が設定値を超えると古い順に破棄されます。  
* MAX\_OK\_IMAGES : 非NG画像名の最大保持数（板毎）（デフォルト：1024）  
...
* HIDE\_CATALOG\_BEFORE\_LOAD : ページ読み込みが完了するまでカタログを隠す（デフォルト：false）  
  ...
  ページの読み込みが完了するまではカタログが表示されませんので、通信状況によっては表示されるのが遅くなる場合があります。  
* USE\_NG\_THREAD\_CLEAR\_BUTTON : スレNG\[クリア\]ボタンを使用する（デフォルト：true）  
...
* USE\_NG\_DISABLE\_BUTTON : NG機能\[一時無効\]ボタンを使用する（デフォルト：true）  
  - NG機能を一時的に無効にするボタンを表示します。  
...
* DISTANCE_THRESHOLD : NG判定する画像の近似度（デフォルト：3）
  - 近似度（ハミング距離）は0～49の値を取り、値が小さいほど画像の近似度が高くなります。  
    画像サイズが「小」のとき、近似度が0で画像のdHash値が完全に一致、1～2でほぼ同じ画像、3～4で近似画像の可能性が高いが稀に誤検出、5～7で近似画像の可能性が有るが誤検出も多いといった感じです。  
* ENABLE_DHASH_TEST : 近似画像NGのテストモードを有効にする。（デフォルト：false）
  - 近似画像NGにしたスレを隠さずにスレ画像を透過して、スレの背景色を変えます。スレ画像にマウスを乗せると透過が解除されます。\[NG\]ボタンにマウスオーバーするか、または開発ツール\(Ctrl + Shift + I\)のコンソールで近似画像NG判定した画像のdHash値と近似度が確認できます。`DISTANCE_THRESHOLD` の調整にご利用ください。
* CHECK_NG_LIST : NGリストが正常かチェックする。（デフォルト：true）
  - 起動時とNGリスト更新時にNGリストが正常かチェックします。異常があった場合は「NGリストが壊れています」と表示されて、編集中の場合は更新できなくなります。
...
* 画像NGの判定負荷が重いため、リロードしたときの動作が重い場合は USE\_NG\_IMAGES を `false` に設定して画像NG機能を無効にしてください。
* 環境によっては「ページ読み込みが完了するまでカタログを隠す」を有効にしてもリロード時にカタログが一瞬表示されることがあります。
* カタログの画像サイズが1~5の状態でv1.6.1以前にNG登録した画像はv1.9.2以降ではNG判定されません。NG画像の再登録をお願いします。
* ふたクロのNG機能と併用できますが\[NG\]ボタンは futaba catalog NG の物が表示されます。
* 画像NGの動作に問題が無いのに「NGリストが壊れています」と表示されてNGリストが更新が出来ないときは`CHECK_NG_LIST`を`false` に設定してください。
...
* v1.10.0 2022-09-30
  - NG機能\[一時無効\]ボタン追加
  - 起動時とNGリスト更新時にNGリストが正常かチェックする機能を追加
  - カタログのタブを閉じてもスレNGをクリアしないように変更
  - テストモード時のハミング距離とdHash値表示のマウスオーバー対象を近似NG画像から\[NG\]ボタンに変更
  - NGリストにNG画像の登録があるとNG画像機能の設定が無効でも動作してしまう不具合を修正
  - その他細かい修正
* v1.9.3 2022-09-27
  - NG画像の最大登録数の設定を追加
  - テストモード時、近似NG画像にマウスオーバーでハミング距離とdHash値を表示
  - デフォルト値を変更
  - その他細かい修正
* v1.9.2 2022-09-13
  - ハミング距離の計算の不具合を修正
  - 近似画像NGのテストモードで検出した画像のマークを修正
  - v1.6.1以前に画像の一部でNG判定していた不具合の対策コードを削除
  - 軽微な不具合を修正
* v1.9.1 2020-03-03
