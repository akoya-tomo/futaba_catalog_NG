...
* \(New\) ENABLE_DHASH_TEST : 近似画像NGのテストモードを有効にする。（デフォルト：false）
  - 近似画像NGにしたスレを隠さずにスレ画像を赤破線で囲み、スレの背景色を変えます。開発ツール\(Ctrl + Shift + I\)のコンソールで近似画像NG判定した画像のdHash値と近似度が確認できます。`DISTANCE_THRESHOLD` の調整にご利用ください。
